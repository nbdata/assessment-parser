...
fputcsv($fp, array('ID', 'Street', 'Non-Homestead', 'Account', 'Code', '2016 City', '2016 Full', '2017 City', '2017 Full', '2018 City', '2018 Full', 
  ...
  '20162022Diff', 'SaleAssessDiff','SaleAssessDiffPercent',
...
foreach ($entries AS $id => $data) {
  ...
  fputcsv($fp, array(
    ...
    $data['2022taxvalue'],
    $data['2022fullmarket'],
    (!empty($data['saledate']) && strtotime($data['saledate'])) ? date('Y-m-d', strtotime($data['saledate'])) : '',
    !empty($data['saleprice']) ? $data['saleprice'] : '',
    !empty($data['saleassess']) ? $data['saleassess'] : '',
    ...
    $data['longitude'],
    $data['tiger'],
    (!empty($data['latitude']) && !empty($data['longitude'])) ? round(greatCircleDistance($data['latitude'], $data['longitude']), 0) : 0,
    (isset($data['2016fullmarket']) && isset($data['2022fullmarket'])) ? round((int) $data['2022fullmarket'] - (int) $data['2016fullmarket']) : '',
    (!empty($data['saleprice']) && !empty($data['saleassess'])) ? round((int) $data['saleprice'] - (int) $data['saleassess'], 2) : '',
    (!empty($data['saleprice']) && !empty($data['saleassess'])) ? round(( (int) $data['saleprice'] - (int) $data['saleassess'] ) / (int) $data['saleassess'], 2) : '',
    $data['sqft'],
    !empty($data['bathrooms']) ? $data['bathrooms'] : '',
    !empty($data['bedrooms']) ? $data['bedrooms'] : '',
    !empty($data['year_built']) ? $data['year_built'] : '',
    !empty($data['land_size']) ? $data['year_built'] : '',
    $data['zoning'],
    $data['ward'],
    $data['census_block'],
    ((int) $data['sqft'] > 0 && !empty($data['2022fullmarket'])) ? round((int) $data['2022fullmarket'] / (int) $data['sqft'], 2) : '',
    (!empty($data['2016fullmarket']) && !empty($data['2022fullmarket'])) ? round(((int) $data['2022fullmarket'] - (int) $data['2016fullmarket']) / (int) $data['2016fullmarket'], 2) : '',
    ((int) $data['sqft'] > 0 && !empty($data['saleprice'])) ? round($data['saleprice'] / (int) $data['sqft'], 2) : '',
    $data['nbhd'],
